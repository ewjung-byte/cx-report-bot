...
const PRODUCT_NAME = {
  ...
  '51': '찬밥 공구',
  '52': '메가쇼 특가 (판매중지)',
  // ★광고 랜딩 상품번호 정본 (2026-08-27 버팀=미주 클로드 확인, 2026-08-20 확정 지도)
  //   전에는 88을 "구글광고(유튜브)"로 뭉뚱그렸는데 실제로는 캠페인마다 상품이 다르다.
  //   틱톡(124)·메타 테스트(127)·DG 신규(128)·DG 리타게팅(129)은 이름표가 아예 없어
  //   보고에 "클래식 바질페스토 150g #127" 처럼 떠서 어느 광고 건지 알 수 없었다.
  //   ⚠️구글 검색은 전용번호가 없다 — 본상품 27 랜딩 + UTM pesto_search 로만 갈린다.
  '83': '메타 광고 (본진)',
  '84': '메타 광고 (본진)',
  '85': 'LG 임직원 특가',
  '87': '꿀동이 공구',
...
function buildDailySnapshot(date, { clarity, dailyOrders, segments, meta, adAudit, pageStats, ltv, ga4Daily }) {
  ...
  return {
    ...
    사이트_메타인앱: clarity?.env?.inAppMetaPct || 0,
    사이트_정상브라우저: clarity?.env?.normalPct || 0,
    사이트_삼성인터넷: clarity?.env?.samsungPct || 0,
    사이트_크롬: clarity?.env?.chromeMPct || 0,
    사이트_사파리: clarity?.env?.safariMPct || 0,
    사이트_모바일: clarity?.env?.mobilePct || 0,
    사이트_PC: clarity?.env?.pcPct || 0,
    사이트_스크롤깊이: clarity?.scrollDepth ? Math.round(clarity.scrollDepth) : 0,
    사이트_체류초: clarity?.activeTimeSec || 0,
...
[CX 개선 대기열 — Jung 영역, 진행 시 매출 직결. 어제 데이터와 맞물리는 1개만 surface. ★예상 임팩트 수치는 미검증 추정이라 출력에 쓰지 말 것]
1. **장바구니 페이지 데드클릭 진단·수정** — Clarity 세션 5개 보고 막힘 위치 찾기. 트리거: /order/basket 데드 10%↑ 또는 뒤로 50%↑ (현재 누수 미확인 상태라 트리거 실제 초과 시에만)
2. **인스타 인앱→사파리 fallback 배너** — 인앱 감지 시 상단 1줄 "더 빠른 결제: 사파리에서 열기". 트리거: 인스타 인앱 50%↑ AND 그날 인앱 데드클릭 5%↑ (둘 다)
3. **상품 페이지 사회적 증거+퍼스트뷰** — 위 fold에 "320명 구매·평균 4.6점" + 컨셉샷. 트리거: 상품페이지 스크롤 50%↓ 또는 뒤로 30%↑
※ ❌**게스트→회원 5천원 쿠폰 후크는 하지 않기로 결정됨(2026-06).** 마진+멤버십/재구매할인 이미 있고, 게스트 대부분이 외부결제(네이버페이/카카오)라 자사몰 주문완료·온사이트 후크가 안 닿음(구조적). 어떤 형태로도 추천 금지.

※ 위 대기열 중 어제 신호와 직결되는 것이 있으면(=해당 트리거 실제 초과) 액션을 **"[항목명] 빌드 시작 — [구체 위치/단계]"** 형태로 그날 1순위 신호의 액션으로 surface. 직결 안 되면 surface X.
...
- "데이터 미수집"은 신호 X
- ❌ 게스트 비중·재방문률 낮은 건 🚨/액션으로 만들지 마: 게스트 대부분이 외부결제(네이버페이·카카오)라 자사몰 주문완료·온사이트 회원전환 후크가 안 닿음 = 자사몰서 못 고치는 구조(측정만). 회원 전환 후크류 액션 추천 금지.
...
- 자사몰 UI 수정처럼 보이는 액션이면 무조건 Jung. "상세페이지 수정"·"장바구니 진단"·"상품페이지 사회적증거" 같은 건 전부 Jung.

모두 정상이면 한 줄: ✅ 특이사항 없음 — <오늘 본업 한 줄>

규칙:
- ★**절대규칙(추측 금지)**: 위 데이터에 실제로 찍힌 측정값만 근거로 써라. 측정 안 된 것을 사실처럼 단정하거나, 측정 없이 액션을 정하지 마(예: "취약 환경"·"누수 의심"·"~일 것"). 액션을 제시하려면 반드시 그 근거 숫자를 같이 대라. 근거 숫자 없으면 "측정 필요"로만. 인과 추측("~때문"·"~로 보임"·"실수로"·"~수 있다")도 금지 — 원인 모르면 "원인 미상, 측정 필요".
